...
class InvestmentsPageViewModel @Inject constructor(
    ...
    val myInvestments =
        mutableStateOf(emptyList<CombinedInvestments>())
    val searchedStocks =
        mutableStateOf(emptyList<Symbol>())
    val networkStatus =
        networkRepo.networkStatus
    val prices =
        mutableStateOf(emptyList<Double>())
    val doneLoading =
    ...
    suspend fun init() {
        ...
        myInvestments.value =
            emptyList()
        val investments =
            investmentsRepo.getInvestments()

        // group investments by symbol
        val mappedInvestments =
            investments.groupBy { it.symbol }
    ...
    suspend fun searchStock(query: String) {
        ...
        searchedStocks.value =
            stocksRepo.searchSymbol(query).distinctBy {
                    listOf(
                        it.symbol,
                        it.instrumentName,
                        it.instrumentType
